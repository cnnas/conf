...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  var udp = Pudp == 1 ? "udp-relay=true, " : "udp-relay=true, "; // 不支持 vmess 类型 udp
  var tmpaead = "aead=false, "; // 不支持 vmess 类型 udp
  node = ip + tfo + udp + tmpaead
  var obfs = ""
...
function SVmess2QX(content) {
    ...
    var pmtd = "method=chacha20-poly1305";
...
function CV2QX(cnt) {
  ...
  mtd = "method="+ "chacha20-poly1305" //cnt.cipher
